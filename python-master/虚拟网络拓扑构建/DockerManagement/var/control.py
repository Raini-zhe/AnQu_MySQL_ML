...
import sys
sys.path.append('/home/ovsdoc/Documents/DockerManagement')
		...
		try:
			for operation in operations:
				...
				for num in range(operation["create_num"]):
					...
					#此处分配ip 操作
					run = RunCommand()
					run.runCommand('pipework '+switch_id+' '+docker_id+' '+ip)
					print "已分配ip：",ip
		...
		try:
			for docker in operation["docker_list"]:
				if docker["host"]!=host_ip:
					...
					self.client = Client(base_url=self.host)
				try:
					self.client.remove_container(container=docker["name"],force=True)
					self.mongo.del_tpl("docker_info",{"docker_id":docker["name"]})
				except:
